...
app.post("/signed", function (req, res) {
  ...
  if (calculated !== signature) {
    ...
    res.send(
      `received: username: ${req.body.username} with password: ${req.body.password}`,
    );
  }
});
...
app.get("/path/:user/login", function (req, res) {
  let encUsername = decodeURIComponent(req.params.user);

  let username = decryptAES(encUsername.toString(), "secret");
